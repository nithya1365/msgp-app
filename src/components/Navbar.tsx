...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      setIsScrolled(window.scrollY > 0); // true as soon as you scroll
    };
        ...
        <div className="bg-white text-gray-900 py-2 px-4 md:px-8">
          <div className="container mx-auto flex justify-between items-center text-sm">
            ...
            <div ref={rightIconsRef} className="flex items-center gap-3 min-w-[220px] justify-end">
              <img ref={truckRef} src="src/assets/truck.jpeg" alt="Truck" className="h-6 w-auto select-none pointer-events-none" />
              <a ref={addIconRef} href="#" className="hover:text-secondary transition-colors"><Facebook size={16} /></a>
              <a ref={addIconRef} href="#" className="hover:text-secondary transition-colors"><Instagram size={16} /></a>
              <a ref={addIconRef} href="#" className="hover:text-secondary transition-colors"><Linkedin size={16} /></a>
              <a ref={addIconRef} href="#" className="hover:text-secondary transition-colors"><Youtube size={16} /></a>
            </div>
          </div>
        </div>

        {/* Main Navbar */}
        <nav className={`transition-all duration-300 bg-green-600 text-white ${isScrolled ? "shadow-lg" : ""}`}>
          <div className="container mx-auto px-4 md:px-8">
            <div className="flex justify-between items-center py-4">
              {/* Logo */}
              <a href="#home" className="text-2xl font-bold text-primary transition-transform hover:scale-105">
              ...
              <div className="hidden lg:flex gap-8 items-center">
                {navLinks.map((link) => (
                  <a key={link.name} href={link.href} className="relative text-white hover:text-white transition-all duration-300 font-medium group hover:text-lg">
                    {link.name}
                  </a>
                ))}
              </div>
